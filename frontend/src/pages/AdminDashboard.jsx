...
export default function AdminDashboard() {
  ...
  const loadLocalDummyState = () => {
    setVehicles([
      { _id: 'v_1', name: 'Suzuki Swift Dzire ("Ayyappa")', type: 'car', seats: 5, ratePerKm: 12, isAvailable: true, image: 'https://images.unsplash.com/photo-1549399542-7e3f8b79c341?q=80&w=600&auto=format&fit=crop', features: ['AC', 'Ayyappa Sticker', 'Neat Cabin'], description: 'Comfortable family sedan.' },
      ...
      { _id: 'v_4', name: 'Mahindra Supro Van (Vel)', type: 'car', seats: 8, ratePerKm: 13, isAvailable: true, image: 'https://images.unsplash.com/photo-1511919884226-fd3cad34687c?q=80&w=600&auto=format&fit=crop', features: ['Traditional Decals', '8 Seats'], description: 'Sacred pilgrimage van.' },
    ]);
    setBookings([
  ...
  const handleAddVehicle = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        image: newVehicle.image || 'https://images.unsplash.com/photo-1549399542-7e3f8b79c341?q=80&w=600&auto=format&fit=crop',
      };
                        ...
                        <tr key={b._id} className="border-b border-slate-100 hover:bg-slate-50">
                          ...
                          <td className="py-4 text-right">
                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${
                              b.status === 'pending' ? 'bg-orange-50 text-brand-600' :
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-left">
                {vehicles.map((v) => (
                  <div key={v._id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-md flex items-start space-x-4">
                    <img
                      src={v.image || 'https://images.unsplash.com/photo-1549399542-7e3f8b79c341?q=80&w=400&auto=format&fit=crop'}
                      alt={v.name}
                      ...
                      <div className="flex items-center space-x-2 pt-1">
                        <button
                          ...
                          className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${
                            v.isAvailable ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'
                          }`}
                        >
